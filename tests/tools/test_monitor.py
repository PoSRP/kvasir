...
def _monitor_with_board(formula: str | None = None,
    ...
    ch  = Channel(channel_id=0, name='v', formula=formula)
...
def test_on_stream_frame_default_conversion_to_volts():
    m, board = _monitor_with_board()
    m.on_stream_frame(board, StreamDataFrame(channel_id=0, seq=0, samples=[4095] * 64))
    valid = board.buffers['v'].view()
    valid = valid[~np.isnan(valid)]
    assert len(valid) > 0
    assert valid[0] == pytest.approx(3.3)


def test_on_stream_frame_applies_formula():
    m, board = _monitor_with_board(formula='v * 2')
    m.on_stream_frame(board, StreamDataFrame(channel_id=0, seq=0, samples=[4095] * 64))
    valid = board.buffers['v'].view()
    valid = valid[~np.isnan(valid)]
    assert len(valid) > 0
    assert valid[0] == pytest.approx(6.6)
...
def test_on_stream_frame_routes_per_board():
    """Two boards with identical channel layouts but different formulas -- frames
    on each board's ch_id_map should route to that board's buffer only."""
    cha = Channel(channel_id=0, name='v')
    chb = Channel(channel_id=0, name='v', formula='v * 10')
    ...
    m.on_stream_frame(a, StreamDataFrame(channel_id=0, seq=0, samples=[4095] * 64))
    m.on_stream_frame(b, StreamDataFrame(channel_id=0, seq=0, samples=[4095] * 64))

    va = a.buffers['v'].view()
    va = va[~np.isnan(va)]
    vb = b.buffers['v'].view()
    vb = vb[~np.isnan(vb)]
    assert va[0] == pytest.approx(3.3)
    assert vb[0] == pytest.approx(33.0)
